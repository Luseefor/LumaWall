...
func buildSettingsViewModelsXPC() async -> AnyObject? {
    let bundleID = Bundle.main.bundleIdentifier ?? "app.lumawall.personal.wallpaper"
    ...
    // matching lumawall:// URL in the companion app.
    ...
    var thumbnailByVideoID = [String: URL]()
    ...
    for entry in entries {
        ...
        thumbnailByVideoID[entry.id] = thumbnailURL
    ...
    if items.count >= 2 {
        items.append(makeShuffleItem(
            bundleID: bundleID,
            choiceIdentifier: shuffleChoiceID,
            name: "Shuffle All",
            description: "Rotate through all installed video wallpapers",
            thumbnailURLs: thumbnailURLs,
            defaultFrequency: .onWakeup,
            sortOrder: 1_000,
            contextMenu: addVideoMenu,
        ))
    }

    for (index, playlist) in ExtensionPlaylistLibrary.load().enumerated() {
        let memberIDs = ExtensionPlaylistLibrary.videoIDs(
            forChoiceID: ExtensionPlaylistLibrary.choiceID(for: playlist),
        )
        let playlistThumbnails = memberIDs.compactMap { thumbnailByVideoID[$0] }
        guard !playlistThumbnails.isEmpty else { continue }
        items.append(makeShuffleItem(
            bundleID: bundleID,
            choiceIdentifier: ExtensionPlaylistLibrary.choiceID(for: playlist),
            name: playlist.name,
            description: "Rotate through \(playlistThumbnails.count) selected video wallpapers",
            thumbnailURLs: playlistThumbnails,
            defaultFrequency: ExtensionPlaylistLibrary.defaultFrequency(for: playlist),
            sortOrder: 1_100 + index,
            contextMenu: addVideoMenu,
        ))
    }

    let automationItems: [(id: String, name: String, description: String)] = [
        (
            ExtensionAutomationLibrary.dayNightChoiceID,
            "Day / Night",
            "Automatically follows the time of day"
        ),
        (
            ExtensionAutomationLibrary.appearanceChoiceID,
            "Light / Dark",
            "Automatically follows the Mac appearance"
        ),
    ]
    for (index, automation) in automationItems.enumerated()
    where ExtensionAutomationLibrary.isConfigured(automation.id) {
        let thumbnails = ExtensionAutomationLibrary.memberIDs(for: automation.id)
            .compactMap { thumbnailByVideoID[$0] }
        guard thumbnails.count == 2 else { continue }
        items.append(makeAutomationItem(
            bundleID: bundleID,
            choiceIdentifier: automation.id,
            name: automation.name,
            description: automation.description,
            thumbnailURLs: thumbnails,
            sortOrder: 1_300 + index,
            contextMenu: addVideoMenu,
        ))
    }
    ...
    // reinstall or OS update — a cached empty group hides the LumaWall section
    ...
    let group = SettingsGroup(
        id: groupID,
        items: items,
        localizedName: "LumaWall \u{2014} Video Wallpapers",
        disposability: .none,
        sortOrder: -100,
...
/// LumaWall's own; the system only stores and returns the selected id.
...
private func makeShuffleItem(
    bundleID: String,
    choiceIdentifier: String,
    name: String,
    description: String,
    thumbnailURLs: [URL],
    defaultFrequency: ShuffleFrequencyID,
    sortOrder: Int,
    ...
    let frequencyPicker = MenuPickerOption(
        ...
        defaultValueID: defaultFrequency.rawValue,
...
private func makeAutomationItem(
    bundleID: String,
    choiceIdentifier: String,
    name: String,
    description: String,
    ...
    let thumbnail = Thumbnail.shuffleImages(urls: thumbnailURLs)
    return SettingsItem(
        id: choiceID,
        localizedName: name,
        thumbnail: thumbnail,
